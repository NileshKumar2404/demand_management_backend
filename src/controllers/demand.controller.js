...
const getDemandOrThrow = async (id) => {
    ...
    if (!demand) throw new ApiError(404, 'Demand not found.')
    return demand
}
...
const ensureStatus = (demand, allowedStatuses, message) => {
    if (!allowedStatuses.includes(demand.status)) throw new ApiError(400, message)
}
...
export const createDemand = asyncHandler(async (req, res) => {
    ...
    const departmentDoc = await Department.findOne({ _id: department, isActive: true }).lean()
    ...
    return res.status(201).json(new ApiResponse(201, enrichDemand(result), 'Demand created successfully.'))
})

export const getDemands = asyncHandler(async (req, res) => {
    const {
        department, status, priority, search, overdue,
        assignedTo, createdBy, fromDate, toDate,
        page = 1, limit = 20
    } = req.query
    ...
    if (status) {
        ...
        if (statuses.some((value) => !VALID_STATUSES.has(value))) throw new ApiError(400, 'Invalid status filter.')
        filter.status = statuses.length === 1 ? statuses[0] : { $in: statuses }
    }

    if (priority) {
        const priorities = String(priority).split(',').map((value) => value.trim()).filter(Boolean)
        if (priorities.some((value) => !VALID_PRIORITIES.has(value))) throw new ApiError(400, 'Invalid priority filter.')
        filter.priority = priorities.length === 1 ? priorities[0] : { $in: priorities }
    }

    if (assignedTo) filter.assignedTo = { $regex: String(assignedTo).trim(), $options: 'i' }
    if (createdBy) filter.createdBy = { $regex: String(createdBy).trim(), $options: 'i' }
...
export const assignDemand = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, enrichDemand(demand), 'Demand assigned successfully.'))
})
...
export const acceptDemand = asyncHandler(async (req, res) => {
    ...
    await createHistory({ demand: demand._id, action: 'ACCEPTED', performedBy, previousStatus, newStatus: 'ACCEPTED' })
    await createNotification({
    ...
    return res.status(200).json(new ApiResponse(200, enrichDemand(demand), 'Demand accepted successfully.'))
})
...
export const startDemand = asyncHandler(async (req, res) => {
    ...
    await createHistory({ demand: demand._id, action, performedBy, previousStatus, newStatus: 'IN_PROGRESS' })
    await createNotification({
    ...
    return res.status(200).json(new ApiResponse(
        200,
        enrichDemand(demand),
        action === 'RESUMED' ? 'Demand resumed successfully.' : 'Demand started successfully.'
    ))
})
...
export const holdDemand = asyncHandler(async (req, res) => {
    ...
    await createHistory({ demand: demand._id, action: 'ON_HOLD', performedBy, previousStatus, newStatus: 'ON_HOLD', reason })
    await createNotification({
    ...
    return res.status(200).json(new ApiResponse(200, enrichDemand(demand), 'Demand put on hold successfully.'))
})
...
export const completeDemand = asyncHandler(async (req, res) => {
    ...
    await createNotification({
        ...
        message: isOverdue ? `${demand.demandNumber} was completed after its SLA deadline.` : `${demand.demandNumber} has been completed.`,
    ...
    return res.status(200).json(new ApiResponse(
        200,
        enrichDemand(demand),
        isOverdue ? 'Demand completed with delay.' : 'Demand completed successfully.'
    ))
})
...
export const closeDemand = asyncHandler(async (req, res) => {
    ...
    await createHistory({ demand: demand._id, action: 'CLOSED', performedBy, previousStatus, newStatus: 'CLOSED' })
    await createNotification({
        title: 'Demand Closed',
    ...
    return res.status(200).json(new ApiResponse(200, enrichDemand(demand), 'Demand closed successfully.'))
})
...
export const getDashboard = asyncHandler(async (req, res) => {
    ...
    const [
        ...
        Demand.countDocuments({ department: departmentId, status: { $nin: ['COMPLETED', 'CLOSED'] }, dueDate: { $lt: now } }),
        Demand.aggregate([
            { $match: { department: objectId } },
            { $group: { _id: '$priority', count: { $sum: 1 } } }
        ]),
        Demand.find({ department: departmentId }).populate('department', 'name').sort({ createdAt: -1 }).limit(10),
        Demand.aggregate([
            { $match: { department: objectId, completedAt: { $ne: null } } },
            { $project: { resolutionMs: { $subtract: ['$completedAt', '$createdAt'] } } },
            { $group: { _id: null, averageResolutionMs: { $avg: '$resolutionMs' } } }
        ]),
    ...
    for (const item of priorityCounts) prioritySummary[item._id] = item.count

    const totalResolved = slaCompleted + slaBreached
    const slaCompliancePercentage = totalResolved === 0 ? 100 : Number(((slaCompleted / totalResolved) * 100).toFixed(2))
    const resolutionMs = avgResolution[0]?.averageResolutionMs || 0

    return res.status(200).json(new ApiResponse(200, {
        department: { id: department._id, name: department.name },
        counts: { total, submitted, assigned, accepted, inProgress, onHold, completed, closed, overdue },
        priorityCounts: prioritySummary,
        sla: {
            completedWithinSla: slaCompleted,
            completedAfterSla: slaBreached,
